...
// ============================================================
// 在切换到搜索页之前调用：隐藏淡入元素，把搜索框放到起始位置+起始尺寸
// ============================================================
void SearchPage::prepareForIntro(const QPoint &fromGlobal, const QSize &fromSize)
{
    ...
    QVector<QWidget*> targets = {m_topRow, m_filterLabel, m_filterScroll,
                                  m_availLabel, m_tagRow1, m_tagRow2, m_hintLabel};
    for (auto *w : targets) {
        if (!w) continue;
    ...
    int startH = startW * finalH / finalW;   // 同比例，不变形
...
// ============================================================
// 播放页→搜索页 入场动画（prepareForIntro 已设好初始状态）
// ============================================================
...
void SearchPage::setupUI()
{
    ...
    // 自定义放大镜图标（风格匹配侧边栏）
    {
        ...
        p.end();
        auto *action = m_searchEdit->addAction(QIcon(mag), QLineEdit::LeadingPosition);
    ...
    m_tagRow1 = createTagRow(row1);
    mainLayout->addSpacing(12);
    m_tagRow2 = createTagRow(row2);
