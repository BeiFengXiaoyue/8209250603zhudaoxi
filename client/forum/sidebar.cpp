...
// ============================================================
ForumNavButton::ForumNavButton(int iconType, const QString &text,
...
void ForumNavButton::setActive(bool active)
{
    m_active = active;
    setChecked(active);
    applyStyle();
...
void ForumNavButton::applyStyle()
{
    QString style = R"(
        QPushButton {
            background-color: %1;
...
void ForumNavButton::paintEvent(QPaintEvent *event)
{
...
void ForumNavButton::drawIcon(QPainter &painter, const QRect &rect)
{
    painter.save();
...
// ============================================================
// ForumSidebarBase — 侧边栏基类
// ============================================================
ForumSidebarBase::ForumSidebarBase(QWidget *parent)
    : QWidget(parent)
{
}
...
void ForumSidebarBase::setupItems(const QList<QPair<int, QString>> &items,
                                  int defaultIndex,
                                  const QString &versionText)
{
    setFixedWidth(210);
    setStyleSheet("ForumSidebarBase { background-color: #FFFFFF; border-radius: 15px; }");
    ...
    for (int i = 0; i < items.size(); ++i) {
        bool active = (i == defaultIndex);
        auto *btn = createNavButton(items[i].first, items[i].second, active);
...
ForumNavButton* ForumSidebarBase::createNavButton(int iconType, const QString &text, bool active)
{
    return new ForumNavButton(iconType, text, active, this);
}
