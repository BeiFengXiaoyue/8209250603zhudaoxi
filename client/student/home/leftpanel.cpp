...
// AvatarWidget — 圆形头像
// ============================================================
StudentAvatarWidget::StudentAvatarWidget(const QString &initials, const QColor &bgColor,
                           QWidget *parent)
...
void StudentAvatarWidget::setAvatarPixmap(const QPixmap &pixmap)
{
    m_pixmap = pixmap;
    update();
}

void StudentAvatarWidget::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event);
...
// LeftPanel — 左侧面板
// ============================================================
StudentLeftPanel::StudentLeftPanel(QWidget *parent)
    : QWidget(parent)
{
    setupUI();
}
...
void StudentLeftPanel::setupUI()
{
    ...
    m_avatar = new StudentAvatarWidget("?", QColor("#5B7DB1"));
    ...
    infoLayout->addWidget(createInfoRow("班级", m_classLabel));
    ...
    // 编辑资料按钮点击事件
    connect(m_editBtn, &QPushButton::clicked, this, &StudentLeftPanel::editProfileClicked);
}

void StudentLeftPanel::setUserData(const QString &username, const QString &userClass,
                             const QString &role)
{
    m_username = username;
    m_userLabel->setText(username);
    m_classLabel->setText(userClass);
