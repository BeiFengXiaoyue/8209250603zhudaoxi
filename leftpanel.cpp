...
void LeftPanel::setupUI()
{
    ...
    m_avatar = new AvatarWidget("王", QColor("#4A7C59")); // 教师使用绿色系
    avatarLayout->addWidget(m_avatar);
    mainLayout->addWidget(avatarContainer);

    // 头像下方点缀文字
    auto *greetingLabel = new QLabel("欢迎回来，王老师");
    ...
    m_userLabel = new QLabel("wang_laoshi");
    m_deptLabel  = new QLabel("2101");
    m_roleLabel  = new QLabel("教师");

    infoLayout->addWidget(createInfoRow("用户名", "wang_laoshi"));
    infoLayout->addWidget(createInfoRow("班级", "2101"));
    infoLayout->addWidget(createInfoRow("角色", "教师"));
